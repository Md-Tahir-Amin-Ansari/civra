...
- Persistence durability follow-up in progress: an intermittent disappearance report is being treated as a close-race risk even though direct database inspection showed the reported chat was saved. Civra now tracks queued native writes and intercepts native close requests until they settle; a failed write keeps the window open and surfaces an error instead of silently risking data loss.
- Close-guard correction in progress: the first implementation incorrectly awaited a programmatic native close from inside its own close-request callback, which can deadlock the close cycle. After pending writes settle, Civra now schedules the follow-up close after the callback returns.
