...
- The browser animation-frame APIs are passed through small closures so their native receiver binding is preserved when the batcher invokes them.
- Frontend refactor branch `refactor/frontend-structure` starts from merged `main` after PR #6. The active 3,000-plus-line inline `index.html` was reduced to a small HTML shell, and the inactive scaffold `main.js`/`styles.css` were replaced by active ES modules and CSS. State, Tauri persistence, setup, chat streaming, rendering, dialogs, and event bindings now have separate files; browser-only mockup controls are isolated. Happy DOM behavior tests cover setup, saved-model startup/failure, streaming, cancellation, and ordered persistence; ESLint checks module references. See `docs/FRONTEND-REFACTOR.md`. This branch is structural, with no intentional product behavior or database change. Manual desktop smoke testing is still required before merge.
